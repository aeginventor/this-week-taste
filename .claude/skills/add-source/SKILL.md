...
## 3. **개별 상품 URL이 있는지 먼저 확인한다** — 없으면 붙이지 않는다

2026-08-25에 P0 소스 **셋이 연달아 이것 하나로 막혔다.** 목록 파싱이 다 되고 나서
알면 그 작업이 전부 버려진다. 그러니 목록 HTML을 처음 볼 때 이것부터 본다.

```
목록의 상품 <a>를 본다
  href가 진짜 상품 주소인가?          → 계속
  href="#none" / 모달 / href 없음     → 여기서 멈추고 사람에게 보고
  href는 있는데 GET으로 안 열리는가?   → 실제로 열어보고 상품명이 나오는지 확인
```

7장이 "요약 + 원문 링크"를 요구한다. 목록 URL로 대체하면 방문자가 수백 건 수십 페이지에서
그 제품을 찾아야 하므로 요건을 실질적으로 못 지킨다.

실제로 막힌 셋 (`sources/targets.yml`에 근거가 있다):

| 소스 | 무엇이 문제였나 |
|---|---|
| 이마트24 | 상품 ID조차 없고 링크가 전부 `href="#none"` |
| 메가MGC | 모달(`inner_modal_open`). 목록에 상품 href가 0개 |
| 세븐일레븐 | Fresh Food는 상세가 **POST 전용**. GET은 껍데기만 온다 |

⚠️ **GET으로 열린다고 끝이 아니다.** 세븐일레븐은 상세 URL이 200에 38KB를 주는데
상품 내용이 없었다. **응답에 제품명이 실제로 들어 있는지** 확인할 것.

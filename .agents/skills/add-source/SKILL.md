---
name: add-source
description: 새 수집 소스(마트/편의점/카페/디저트/프랜차이즈/식품사) 하나를 파이프라인에 붙인다. sources/targets.yml 갱신, scrapers/<id>.py 작성, 골든 테스트, pipeline/sources.py 등록까지. "소스 추가", "크롤러 붙이기", "<브랜드> 붙여줘" 같은 요청에 쓴다.
---

# 소스 하나를 붙인다

## 2026-09-09 이후 판별 계약

- [ADR-0018](../../../docs/adr/0018-discovery-and-launch.md)을 적용한다.
- 소스 목록이 무엇을 관측하는지 명시하고 `added`를 출시로 단정하지 않는다.
- 카탈로그 항목 ID와 제품의 동일성을 구분한다. 기존 ID를 먼저 매칭하고 모호한 보조 키를 입력 순서로 소비하지 않는다.
- 새 발견 후보는 과거 성공 관측과 대조한다. 과거 원본 ID 또는 바코드·정규화 이름·알려진 가격이 모두 같으면 근거와 함께 보류한다.
- 이름만 같거나 가격이 없다는 이유로 제품을 합치지 않는다.
- 공식 출처에서 해당 제품의 출시 사실과 시점이 확인될 때만 출시 확인으로 표시한다. NEW 배지와 판매 시작일만으로 확정하지 않는다.
- LLM 편집은 재입고 판별·중복 병합·출시 검증을 수행한다고 가정하지 않는다.
- 라벨 일치율·소실률을 실제 출시 정확도로 보고하지 않는다.
- [ADR-0019](../../../docs/adr/0019-publication-input-integrity.md)에 따라 이전 성공본의 관측·원본·판별·보강·코드 지문을 검증하고 수집 시점을 표시한다.
- 이월본을 새 관측으로 세지 않으며 증명이 없는 옛 부분 파일은 병합에서 제외한다.
- LLM 응답의 중복 참조·타입·채널 분류를 검사하며 문자열 `"false"`로 제품을 제외하지 않는다.
- 과거 공개 ID·URL과 수집 원본을 보존하고 검증 산출물은 격리한다.


AGENTS.md 5장 체크리스트를 실행 순서로 펼친 것이다. 2026-08-26에 피자헛이 붙어
**P0가 전부 소진됐다.** 2026-08-27에 크라운제과가 P1 verified 9곳 중 하나로 붙었는데,
**그 9곳을 "보도자료 소스"로 뭉뚱그려 두었던 것이 틀렸다** — 크라운은 제품 카탈로그를
갖고 있어 이 체크리스트가 그대로 통했다. 남은 8곳(신세계 뉴스룸 + FMCG 7)은 증분
피드라 구조가 다르다. 그 밖에는 blocked·unverified다.

→ **`method: press`라고 적힌 소스도 카탈로그가 있는지 먼저 본다.** `targets.yml`의
`catalog` 필드가 그 신호다.

원칙은 AGENTS.md가 갖는다. 여기 적는 것은 **매번 같은 자리에서 걸리는 것들**이다.

---

## 0. 코드를 쓰기 전에 사람에게 묻는다

7장 "실행 전에 승인을 받는 것"이다. 나중에 물으면 이미 요청이 나가 있다.

- **이용약관** — `robots.txt`는 `base.Session`이 강제하지만 약관은 사람이 읽어야 한다
- **이미지 사용 방식** — 원본 CDN 참조가 기본이지만 소스마다 다시 봐야 한다.
  ⚠️ 오리온은 `robots.txt`가 `/upload/`를 막는데 이미지가 거기 있어서 **이미지를 안 내보낸다.**
  같은 경로라도 호스트가 다르면 해당 없다(스타벅스가 그렇다)

`sources/targets.yml`의 해당 항목을 먼저 읽는다. 정찰(1단계) 결과가 이미 거기 있고,
`docs/RECON_*.md`에 더 자세한 실측이 있다. **정찰이 끝난 소스는 다시 정찰하지 않는다.**

⚠️ **정찰이 "브라우저로 캡처해야 한다"고 적어둔 소스는 그 말을 검증부터 한다.**
버거킹이 그래서 1년 가까이 밀려 있었다 — 요청 봉투를 모른다(빈 body → HTTP 400)는
기록이었는데, **봉투를 만드는 코드가 사이트에 실려 있었다**(`/bizMOB/bizMOB-webExtend.js`).
SPA의 요청 형식은 어차피 브라우저가 JS로 만든다. 그 JS는 우리도 받을 수 있다.
순서는 `앱 번들(app.js·청크) → 프레임워크 JS → 그래도 모르면 브라우저 캡처`다.

## 1. robots.txt를 실측한다

`Crawl-delay`와 `Visit-time`을 본다. **둘 다 `base.Session`이 강제하므로 스크래퍼가
할 일은 없다**([ADR-0014](../../../docs/adr/0014-collection-time-window.md)).
`Visit-time`이 있으면 대신 **수집 cron을 봐야 한다.**

현재 cron은 둘이다 — 시각 제한이 없는 소스는 KST 월 10:00, 창이 걸린 소스는 KST 월 14:00.
새 소스의 창이 둘 중 어느 쪽에도 안 맞으면 워크플로를 다시 갈라야 한다. 사람에게 알린다.

⚠️ **창 밖에서는 그 소스를 로컬에서도 못 돌린다.** `VisitTimeClosed`로 거부된다.
gs25가 그런 소스다(`Crawl-delay: 10`, `Visit-time: 0400-0845 UTC` = KST 13:00~17:45).

## 2. 이 소스의 카테고리 중 **무엇이 우리 범위인가**를 먼저 정한다

**여기가 이 체크리스트에서 제일 자주 빠지는 자리다.** 2026-08-24에 실제로 터졌다 —
CU의 생활용품 838건을 정찰 표에서 그대로 옮겨와 티슈·바디로션·가그린·화장솜·콘돔이
사이트에 실렸고, 홈플러스는 과일·채소·정육·수산물·쌀까지 긁고 있었다.

범위는 **포장된 식품·음료 공산품**(과자, 라면, 냉장·냉동, 레토르트, 통조림, 유제품,
음료, 커피/차, 조미료)과 **프랜차이즈 카페·매장의 신메뉴**다.
범위 밖: 생활용품·잡화, 신선 원물(과일·채소·정육·수산물·쌀), 의약외품,
건강기능식품 정제/캡슐, 반려동물 용품, 식기·조리도구, 완구·캐릭터 상품.

**소스가 주는 전부를 긁지 않는다.** 2.1의 "전체 카탈로그"는 *우리가 추적하는* 카탈로그이지
소스가 파는 전부가 아니다. 거르는 자리는 세 층이고, 위에서부터 쓴다.

| 층 | 어디 | 무엇을 거르나 |
|---|---|---|
| 1 | `scrapers/<id>.py`의 `CATEGORIES` | 소스 카테고리 단위. 결정적이고 **요청 수도 준다** |
| 2 | `curate.py`의 분류 목록 | 범위 밖 라벨을 아예 두지 않는다 |
| 3 | `curate.py`의 `out_of_scope` | 1층으로 못 가르는 잔여. 기본값 `false`(포함) |

⚠️ **키워드로 거르지 마라.** 2026-08-24에 CU에서 실측했더니 `사과|거봉|딸기...` 같은
정규식에 327건이 걸렸는데 대부분 `샌)딸기듬뿍크림치즈샌드`·`토네이도감자` 같은 오탐이었다.

⚠️ **이미 수집하던 카테고리를 빼면 지난주 스냅샷도 같이 정리해야 한다.** 안 그러면
그 주 diff가 전량을 "단종"으로 잡는다. 건수 검증은 안전하다 — `CATEGORIES`에서 사라진
이름은 `_category_counts`의 역매핑에서도 빠져 baseline에 안 잡힌다.

## 3. `source_url`로 무엇을 쓸지 정한다

**개별 상품 URL이 없다고 소스를 보류하지 않는다**([ADR-0013](../../../docs/adr/0013-source-url-optional.md)).
2026-08-25 오전에 이 자리에서 P0 셋을 잘라냈다가, 같은 날 오후에 되돌린 규칙이다.
7장이 금지하는 것은 *통째 재배포*이고 원문 링크는 그 수단인데, 수단을 요건으로
승격시킨 것이 원인이었다. 4장은 처음부터 `source_url`을 nullable로 두고 있었다.

목록 HTML을 처음 볼 때 이것부터 보고, **위에서부터 가능한 것을 쓴다.**

```
목록의 상품 <a>를 본다
  href가 진짜 상품 주소인가?          → 1층. 열어서 제품명이 실제로 들어 있는지 확인
  href="#none" / 모달 / href 없음     → 2층. 목록 페이지 URL을 쓴다
  목록 URL조차 안정적이지 않은가?      → 3층. null로 둔다
```

| 층 | 값 | 실제 사례 |
|---|---|---|
| 1 | 개별 상품 상세 URL | cu, homeplus, starbucks, orion, compose, baskinrobbins, dunkin, kyochon, bbq, burgerking, mcdonalds |
| 2 | **그 제품이 실린 목록 페이지 URL** | emart24 (`/goods/pl`) |
| 3 | `null` | 아직 없다 |

⚠️ **한 소스 안에서 층이 갈릴 수 있다.** 도미노와 피자헛이 그렇다 — 피자헛은
`main.js`의 `cpath`가 분류 다섯만 분기해서, 거기 없는 분류(`CF`)만 2층을 쓴다.
소스 단위로 "이 소스는 몇 층"이라고 적어두면 그 예외가 조용히 틀린다.

⚠️ **GET으로 200이 온다고 1층인 것이 아니다.** 세븐일레븐은 상세 URL이 200에 38KB를
주는데 상품 내용이 없었다. **응답에 제품명이 실제로 들어 있는지** 확인할 것.

⚠️ **서버 응답에 없다고 2층인 것도 아니다.** 클라이언트 렌더링이면 브라우저에서는
정상으로 보인다 — 방문자는 브라우저로 열므로 1층이 맞다. 홈플러스와 BBQ가 그 경우이고,
둘 다 **브라우저로 한 번 열어 확인**하고 채택했다.

**여기서 멈추고 사람에게 보고하는 경우는 하나뿐이다** — 목록에서 **제품명조차 못 얻을 때**.
이름은 4장의 유일한 필수 필드라, 없으면 발행할 것이 없다.

## 4. 전체 카탈로그를 한 번 뜨고 `external_id` 유일성을 실측한다

### 먼저 — **"전체"라고 부르는 응답이 정말 전체인지 확인한다**

**세 번 연속 아니었다.** 던킨 `/menu/all`, 파리바게뜨 `?cat1=`, 피자헛
`/api/menu/pizza/all`. 마지막 것은 이름이 대놓고 `all`인데 실제로는 탭 다섯 중 둘의
합이었고, **빠진 하나가 하필 사이트가 NEW 배지를 달아둔 판촉 탭**이었다(전체의 32%).

거르는 방법은 두 가지고, 위에서부터 쓴다.

| | 방법 | 쓸 수 있을 때 |
|---|---|---|
| 1 | **소스가 밝힌 총건수와 대조**한다 | 응답에 `totalCount` 류가 있을 때 (gs25·던킨) |
| 2 | **브라우저로 목록 화면을 열어 탭·필터를 하나씩 눌러 본다** | 총건수를 안 줄 때 |

2번이 번거로워 보여도 이번엔 그것 말고는 알아낼 길이 없었다. 응답만 봐서는
17건이 완전해 **보였다** — 유일성도 통과하고, 분류도 깔끔하고, 설명문도 17/17이었다.

⚠️ **알아낸 다음이 더 중요하다.** 탭이 여럿이면 `BOOTSTRAP_COUNTS`에 **탭마다 한 줄씩**
넣는다. 한 탭이 통째로 실패해도 나머지가 오면 총건수는 그럴듯하게 남는다 —
분류별 기준이 있어야 2.4가 시끄럽게 실패한다.

### 그다음 `external_id` 유일성

**추측하지 않는다.** 4장이 경고하는 자리이고, 실제로 바코드가 16건 중복된 적이 있다
([ADR-0001](../../../docs/adr/0001-product-id.md)).

- 주키는 *물리적 제품*이 아니라 **카탈로그 항목**을 가리키는 키로 잡는다
- 소스 간 대조에 쓸 수 있는 키(바코드 등)는 `alt_ids`에 넣는다
- 같은 이름이 두 번 나오는지도 여기서 본다. 나오면 이름 해시로는 가를 수 없다
  (오리온이 동명이인 2건이다)

## 5. 설명문이 어디 있는지 판정한다

`enrich` 경로가 여기서 갈린다. 세 갈래이고 **전부 실제로 있다.**

| 어디 | 소스 | `detail` |
|---|---|---|
| 목록 응답이 이미 준다 | starbucks (326/326) | `False` — 가진 것을 버리고 다시 긁지 않는다 |
| 상세에만 있다 | cu, orion | `True` |
| **상세에도 없다** | homeplus (표본 51건 중 텍스트 1건) | `False` — `blurb`는 항상 `null` |

목록이 주면 스냅샷의 `description`을 채운다(4장). 뒤 두 경우는 `null`이다.
**지어내지 않는다**(6장).

## 6. 파일을 고친다 — 다섯 곳

```
sources/targets.yml       status를 verified로, method·url 확정
scrapers/<id>.py          fetch() + parse_*() + CATEGORIES + BOOTSTRAP_COUNTS
tests/test_<id>_parse.py  tests/fixtures/의 저장된 응답으로 골든 테스트
tests/fixtures/           원본 응답 샘플을 커밋한다 (scratch/는 gitignore라 근거를 못 둔다)
pipeline/sources.py       표에 한 줄: brand·channel·detail·monotonic_key
                          (+ 러너에서 막히거나 robots가 시각을 제한하면 collector·windowed)
```

`pipeline/`에서 고칠 곳은 **표 한 줄뿐이다.** 스크래퍼 모듈은 `scrapers/<source_id>.py`
규칙으로 찾으므로 등록이 필요 없다. 카테고리 코드와 부트스트랩 건수는 소스의 *내용*이라
스크래퍼 파일이 갖는다 — `pipeline/`에 두면 7장이 말하는 누수다.

**선례를 그대로 베낀다** (7장: 소스 3개까지 복붙 허용, 4개째에 공통화):

- `scrapers/cu.py` — xhr, 카테고리 7 × 페이지 = 131요청, 상세 있음, `gd_idx` 단조 키
- `scrapers/homeplus.py` — xhr, 카테고리 108개, 가격 있음, 상세에도 설명문 없음
- `scrapers/starbucks.py` — xhr, 목록이 설명문을 준다, 가격 없음
- `scrapers/orion.py` — **static HTML 파싱**, 동명이인 있음, 이미지 미출력
- `scrapers/compose.py` — static, 카테고리별 페이지네이션, 가격·설명문 둘 다 없음
- `scrapers/gs25.py` — **POST**, 세션 토큰, 소스가 밝힌 총건수와 대조, 범위 밖 분류 제외
- `scrapers/burgerking.py` — POST + 고정 요청 봉투, **전체 카탈로그가 요청 1건**,
  한 항목이 여러 분류에 실려 접어야 하는 소스
- `scrapers/pizzahut.py` — GET JSON, **탭 둘을 합쳐야 전체가 되는 소스**, 탭마다
  섞임 허용 여부가 다르다, 분류에 따라 `source_url`이 1층·2층으로 갈린다.
  **가장 최근에 이 체크리스트를 그대로 밟아 만든 것**이라 형식을 베끼기 좋다

각 스크래퍼는 `fetch(*, week) -> list[dict]` 하나를 노출하고,
`python -m scrapers.<id>`로 단독 실행하면 결과를 stdout에 찍는다.
파싱은 네트워크와 분리한다 — 그래야 골든 테스트가 된다.

`monotonic_key`는 **확인되지 않으면 `None`이다.** 틀린 지표는 없는 지표보다 나쁘다
(7장의 `gd_idx_monotonic` 사고).

### 수집 주체와 시각 창 (예외일 때만 적는다)

`collector`와 `windowed`는 **기본값이 아닐 때만** 적는다(각각 `"actions"`, `False`).

- `collector: "local"` — **Actions 러너에서 막히는데 로컬에서는 되는 소스**
  ([ADR-0017](../../../docs/adr/0017-collector-identity.md)). 403·타임아웃·robots
  `Disallow` 셋 다 이 증상으로 나타난다. ⚠️ **로컬에서 되는지 먼저 확인하고 적는다** —
  양쪽에서 다 막히면 그건 소스 쪽 변화이지 수집 주체 문제가 아니다
- `windowed: True` — robots가 `Visit-time`으로 수집 시각을 제한하는 소스
  ([ADR-0014](../../../docs/adr/0014-collection-time-window.md))

이 둘을 잘못 적으면 **그 소스가 어느 실행 묶음에도 들지 않아 조용히 수집에서 빠진다.**
`tests/test_sources.py`의 "모든 소스가 정확히 한 묶음에 든다"가 이것을 잡는다.
워크플로는 고치지 않는다 — 묶음에 무엇이 드는지는 표가 정한다.

## 7. 새 채널이면 분류 목록을 만들고 **돌려서 확인한다**

`pipeline/curate.py`의 `CATEGORIES_BY_CHANNEL`에 그 채널이 없으면
`system_prompt()`가 예외를 던지고 `curate()`가 LLM을 건너뛰어,
**그 소스 전량이 원본 그대로 발행된다**(조용하다 — 발행은 성공한다).

2026-08-25에 **6채널을 전부 채웠으므로** 지금 붙는 소스는 대개 해당 없다.
새 채널이 생길 때와 **기존 목록을 고칠 때**만 이 절을 본다.

⚠️ **다만 "고칠 때"인지는 돌려 보기 전에는 모른다.** 2026-08-27에 크라운제과를
fmcg에 붙였는데, 그 목록은 오리온만 보고 만든 것이라 **케이크류가 통째로 빠져
있었다.** 예행에서 `롱스`와 `크림블 크렘브륄레맛`이 `기타`로 떨어져서 알았다.
→ **소스의 탭 이름을 채널 목록과 눈으로 대조해 보고, 갈 곳이 없는 탭이 있으면
그때 예행을 돌린다.** 크라운은 소스 자체가 `케이크` 탭을 갖고 있었다.

목록은 **실제 카탈로그를 보고** 만든다([ADR-0006](../../../docs/adr/0006-category-taxonomy-per-channel.md)).
편의점 목록을 재사용하면 분류가 뭉개진다 — 오리온 20건이 전부 `과자`로 떨어져
사이트 필터가 아무 일도 하지 못했던 것이 그 예다.

### 만들었으면 표본으로 돌려 본다

```bash
THIS_WEEK_TASTE_DATA_DIR=<비공개 저장소> THIS_WEEK_TASTE_LLM=cli \
  python -m scripts.curate_dryrun --channel <채널> --week <주차> \
    --weight <새로_붙은_소스>=12
```

발행하지 않고 스냅샷만 읽는다. 보는 것은 넷이다 — `기타` 비율, 목록 밖 값,
한 칸 쏠림, `name` 변조. 2026-08-26 실측(restaurant 40 / dessert 40)에서는
`기타` 1/80, 목록 밖 0, 변조 0이 나와 **목록을 고치지 않았다.**

⚠️ **표본을 그냥 뽑지 마라.** 목록을 만든 뒤에 붙은 소스가 있으면 `--weight`로
그쪽을 채운다. restaurant 목록은 소스 6곳을 보고 만들었는데 그 뒤 버거킹·피자헛이
붙어서, 예행 40건 중 18건을 그 둘로 채웠기 때문에 새 소스가 목록에 맞는지 알 수 있었다.

⚠️ **분포는 품질이 아니다.** 던킨의 `블렌디드/빙수` 4/12는 숫자만 보면
"도넛집인데 왜?"였는데 이름을 보니 비타슬러시·망고쿨라타·컵빙수라 전부 맞았다.
스크립트가 항목별 목록을 함께 찍는 이유다 — **거기를 봐야 오분류가 보인다.**

⚠️ **`기타`가 많다고 목록 탓부터 하지 마라.** 예행의 유일한 `기타`
(파리바게뜨 `쇼콜라티 실론`)는 이름만으로는 사람도 모르는 것이었다.
`detail: True`인 소스는 예행에서 설명문이 비므로 실제 발행보다 불리하게 나온다.

**나오지 않은 문제를 미리 고치지 않는다.** 그 수정에는 근거가 없다.

## 8. 공유 코드가 이 소스의 모양을 견디는지 답한다

5장의 네 질문이다. **테스트는 이미 겪은 것만 지킨다.** 답이 "그렇다"면 그때 테스트를 쓴다.

- `diff.py`가 이 소스의 `alt_ids` 키 이름을 하드코딩하지 않는가
- **가격이 없는 소스인가** — 그러면 diff의 (이름, 가격) 계층이 무력해진다
  (starbucks, orion이 그렇다)
- **같은 이름이 두 번 나오는가** — 이름 해시로 가를 수 없다
- **카테고리가 없는 소스인가** — `snapshot.py`의 건수 검증이 카테고리를 전제한다

## 9. 첫 수집

```bash
make test
THIS_WEEK_TASTE_DATA_DIR=<비공개 데이터 저장소> make snapshot SOURCE=<id>
```

**첫 스냅샷은 발행되지 않는다.** 비교 대상이 없어 구조적으로 불가능하다 —
그래서 8장은 소스 추가를 미루지 말라고 한다. 첫 주는 `BOOTSTRAP_COUNTS` ±10%로
검증되고, 그 다음 주부터 직전 주의 30~300%로 바뀐다([ADR-0002](../../../docs/adr/0002-category-count-baseline.md)).

⚠️ 같은 주차를 다시 뜨려면 `REFRESH=1`이다. 스냅샷이 있으면 다시 긁지 않는다
(2.6, [ADR-0011](../../../docs/adr/0011-snapshot-once-per-week.md)).

## 마지막에 보고한다

- 스스로 정한 것(파서 구현, 파일 배치, 카테고리 코드, 부트스트랩 건수)은 **ADR로 보고**
- 4장 스키마를 건드려야 하면 **먼저 묻는다**(8-1)
- 커밋은 코드와 데이터를 섞지 않는다(9장). 메시지는 커밋 전에 상의한다

...
3. **소스가 준 태그는 LLM이 실패해도 살아남는다** (4장 `tags` 계약)

id가 바뀌면 아카이브 URL이 깨지고 diff가 같은 제품을 "단종 1건 + 신상 1건"으로 오탐한다.

3번이 여기 있는 이유: 2026-08-28까지 `_publish_item`이 `curated`만 보고 있어서
LLM이 실패하면 배스킨라빈스가 목록으로 준 태그 128건이 통째로 사라졌다.
**발행은 성공하고 카드에서 태그만 없어지므로 조용하다** — 7장이 말하는
"틀린 답이 예외 없이 그냥 나오는 코드"다.
"""
...
def test_new_item_gets_id_and_first_seen_from_this_week():
    item = publish._publish_item(snapshot_item("17620"), week="2026-W33",
                                 source_id="cu", curated={}, enriched={}, previous=None)
...
def test_id_and_first_seen_are_carried_forward_never_recomputed():
    ...
    item = publish._publish_item(snapshot_item("17620"), week="2026-W33",
                                 source_id="cu", curated={}, enriched={}, previous=previous)
...
def test_curated_fields_applied_but_name_is_never_touched():
    ...
    item = publish._publish_item(snapshot_item("17620"), week="2026-W33",
                                 source_id="cu", curated=curated, enriched={}, previous=None)
...
# ── 태그 폴백 (4장의 `tags` 계약) ────────────────────────────────
#
# 태그의 출처가 셋이다. LLM이 다듬은 것, enrich가 합친 것, 스냅샷이 가진 것.
# **소스가 주는 태그는 LLM의 산물이 아니므로 LLM이 실패해도 남아야 한다.**


def test_소스가_준_태그는_LLM이_실패해도_남는다():
    """`curated`가 비는 것은 LLM 실패의 정상 경로다 (6장). 발행은 계속되고 조용하다."""
    item = publish._publish_item(snapshot_item("17620", tags=["크림치즈", "조청카라멜"]),
                                 week="2026-W33", source_id="cu",
                                 curated={}, enriched={}, previous=None)
    assert item["tags"] == ["크림치즈", "조청카라멜"]


def test_LLM이_태그를_빼먹어도_소스_태그가_남는다():
    """분류·blurb는 왔는데 tags 키만 없는 경우. 부분 실패도 실패다."""
    item = publish._publish_item(snapshot_item("17620", tags=["크림치즈"]),
                                 week="2026-W33", source_id="cu",
                                 curated={"17620": {"category": "샐러드", "blurb": "x"}},
                                 enriched={}, previous=None)
    assert item["tags"] == ["크림치즈"]


def test_enrich가_합친_태그를_스냅샷보다_먼저_쓴다():
    """enrich는 상세 태그와 스냅샷 태그를 이미 합쳐뒀다. 그쪽이 더 많다."""
    item = publish._publish_item(snapshot_item("17620", tags=["스냅샷만"]),
                                 week="2026-W33", source_id="cu", curated={},
                                 enriched={"17620": {"description": "설명",
                                                     "tags": ["상세", "스냅샷만"]}},
                                 previous=None)
    assert item["tags"] == ["상세", "스냅샷만"]


def test_LLM_태그가_있으면_그것을_쓴다():
    item = publish._publish_item(snapshot_item("17620", tags=["원본"]),
                                 week="2026-W33", source_id="cu",
                                 curated={"17620": {"tags": ["다듬은것"]}},
                                 enriched={"17620": {"description": None, "tags": ["보강"]}},
                                 previous=None)
    assert item["tags"] == ["다듬은것"]


def test_아무데도_없으면_빈_목록이다():
    """지어내지 않는다 (6장). null이 아니라 []다 (4장)."""
    item = publish._publish_item(snapshot_item("17620"), week="2026-W33",
                                 source_id="cu", curated={}, enriched={}, previous=None)
    assert item["tags"] == []

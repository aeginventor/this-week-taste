...
LLM 폴백 뒤 확인된 범위 밖 상품은 `catalog_exclusions`에 정확한 `id`, `name`, `source_id`, `source_url`,
`reason`, `reviewed_by`, `evidence`를 기록한다. 병합에서만 제외하며 기존 스냅샷·부분 파일은 유지한다.
리포트의 `by_source` 발행 집계는 부분 파일 기준이고, `review_excluded`와 최종 `totals`가 검토 제외를 설명한다.
2026-09-10 실제 CLI 호출은 조직의 구독 접근 차단(403)으로 실패했다. 로그인 표시만으로 사용 가능하다고 판단하지 않는다.
API 키는 없었으며 임의로 다른 유료 모델을 켜지 않았다. 해제 또는 별도 API 환경이 마련되기 전에는 편집 자동화 완료로 보고하지 않는다.

## 다음 실제 실행의 후속 확인

2026-09-10 현재 작업에 ‘주간 수집 실행 결과 확인’ 후속 자동화(`automation-2`)를 설정했다.
월·화 KST 21:30에 실행하며 첫 예정 확인은 9/14 월요일 W38 수집 뒤다. 원격 실행 유무·소스별 결과·
산출물 주차를 대조하고 첫 결과·새 실패·복구·조치 필요 때 알린다. 화요일에 같으면 반복 알림을 보내지 않는다.
이는 로컬 앱 후속 확인이므로 기계와 앱이 켜져 있고 프로젝트·GitHub 인증을 읽을 수 있어야 한다.
[공식 실행 조건](https://learn.chatgpt.com/docs/automations?surface=app)을 확인했다. 다음 주 실제 실행은 아직 미래이며 성공 여부는 미확인이다.

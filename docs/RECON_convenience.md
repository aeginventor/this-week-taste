...
- 상품 상세 링크가 `href="#none"`이다. **`source_url`로 쓸 개별 상품 URL이 없다.**
  → **판단 완료(2026-08-25, [ADR-0013](adr/0013-source-url-optional.md))** — 목록 페이지
  URL로 대체하고 붙였다. 7장이 금지하는 것은 *통째 재배포*이고 원문 링크는 그 수단인데,
  우리가 발행하는 것은 주당 수십~수백 건의 차집합이라 애초에 재배포가 아니다.
